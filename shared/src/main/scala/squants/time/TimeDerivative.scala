 ...
 */
trait TimeDerivative[A <: Quantity[A] & TimeIntegral[?]] {
...
trait SecondTimeDerivative[A <: SecondTimeIntegral[?]] { self: TimeDerivative[?] =>
  protected[squants] def time: Time
 ...
 */
trait TimeIntegral[A <: Quantity[A] & TimeDerivative[?]] {
