...
object SVector {
  ...
  def apply(coordinates: Double*): DoubleVector = DoubleVector(coordinates*)
  def apply[A <: Quantity[A]](coordinates: A*): QuantityVector[A] = QuantityVector[A](coordinates*)
...
case class DoubleVector(coordinates: Double*) extends SVector[Double] {
   ...
   * @return
   */
  def map[A <: Double](f: Double => Double): DoubleVector = DoubleVector(coordinates.map(f)*)
...
case class QuantityVector[A <: Quantity[A]](coordinates: A*) extends SVector[A] {
   ...
   * @return
   */
  def map[B <: Double](f: A => Double): DoubleVector = DoubleVector(coordinates.map(f)*)
